...
namespace bfbd.Common
{
	...
	partial class Database
	{
			...
			where T : new()
		{
			string sql = MakeSelect(tableName, condition, columns);
			DataTable dt = InvokeTableQuery(sql, condition);
			return (dt.Rows.Count > 0) ? DataConverter.Convert<T>(dt.Rows[0]) : default(T);
		}
			...
			where T : new()
		{
			...
			foreach (DataRow row in dt.Rows)
				yield return DataConverter.Convert<T>(row);
		}
...
#if _DEBUG
		public T[] SelectObjects<T>(string tableName, string condition, string order, int group, params string[] columns)
			where T : new()
		{
			List<T> array = new List<T>();
			Debug.Assert(group < columns.Length);
			...
			DataTable dt = InvokeTableQuery(sql, condition);
			foreach (DataRow row in dt.Rows)
				array.Add(DataConverter.Convert<T>(row));
			return array.ToArray();
		}

		public T[] SelectObjects<T>(string tableName, object condition, string order, int group, params string[] columns)
			where T : new()
		{
			List<T> array = new List<T>();
			Debug.Assert(group < columns.Length);
