...
{	
	...
	// start translation always,
	// user must call submit to complete a translation,
	// if not submit, translation will cancel at dispose or finallier.
...
exception : {2}", sql, sb.ToString(), ex.Message));
	...
	partial class DatabaseEngine
	{
		...
		private static string MakeGroup(string[] columns, int group)
		{
			...
			{
				//sql = "GROUP BY " + string.Join(",", columns.Take(group).Select(c =>
				//{
				...
				{
					var c = columns[i];
					cols.Add(c.Contains(" ") ? c.Substring(0, c.IndexOf(' ')) : c);
				}
